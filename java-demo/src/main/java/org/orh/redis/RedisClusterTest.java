...
public class RedisClusterTest {
    // 集群六个节点都在 192.168.1.25 [7001-7006]上
    String clusterString = "192.168.1.25:7001,192.168.1.25:7002,192.168.1.25:7003,192.168.1.25:7004,192.168.1.25:7005,192.168.1.25:7006";

    JedisCluster jedisCluster = null;

    static String prefix = "orh:test";
    ...
    public void before() {
        ...
        String[] hostAndPorts = clusterString.split(",");
        Arrays.stream(hostAndPorts)
                .map(s -> s.split(":"))
                .forEach(arr -> nodes.add(new HostAndPort(arr[0].trim(), Integer.valueOf(arr[1].trim()))));

        // 初始化，设置超时时间
        jedisCluster = new JedisCluster(nodes, 1000, 1000, new GenericObjectPoolConfig());

        // nameKey 作为测试使用，启动时删除掉
    ...
    public void psubscribe() {
        jedisCluster.psubscribe(new JedisPubSub() {
            @Override
            public void onMessage(String channel, String message) {
                System.out.println("channel1 on message:" + message + "(" + channel + ")");
            }
        }, "channel1");
    }
